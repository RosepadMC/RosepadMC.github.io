const { compile, renderFile } = require("ejs");
...
async function build() {
  ...
  for (const news of await readdir(`${__dirname}/news`)
             .then(a => a.sort((a, b) => Number(a.split(".")[0]) - Number(b.split(".")[0])))) {
    const [title, description, ...paragraphs] = await readFile(`${__dirname}/news/${news}`, 'utf8')
    ...
    await writeFile(`${__dirname}/build/news/${news.replace(/\.txt/, ".html")}`, string);
  }
